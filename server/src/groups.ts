...
export class Groups implements GroupsService {
    ...
    public async addUserToGroups(user: string, groups: string[], audit: AuditBuilder): Promise<void> {
        const localUser = this.findUser(user);
        localUser?.groups.push(...groups);
    }

    public async removeUserFromGroups(user: string, groups: string[], audit: AuditBuilder): Promise<void> {
        const localUser = this.findUser(user);
    ...
    public async removeAll(arg0: AuditBuilder): Promise<void> {
        groups.clear();
    }
